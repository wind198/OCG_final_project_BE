...
CREATE TABLE `order_details` (
  ...
  `product_variance_id` bigint(20) unsigned DEFAULT NULL,
  `quantity` float DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idx_order_details_deleted_at` (`deleted_at`),
  KEY `fk_orders_order_details` (`order_id`),
  KEY `fk_product_variances_order_detail` (`product_variance_id`),
  CONSTRAINT `fk_orders_order_details` FOREIGN KEY (`order_id`) REFERENCES `orders` (`id`),
  CONSTRAINT `fk_product_variances_order_detail` FOREIGN KEY (`product_variance_id`) REFERENCES `product_variances` (`id`)
